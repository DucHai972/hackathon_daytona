...
class HostGit:
    ...
    def apply_and_commit(
        ...
        self._run(["git", "config", "user.name", "AutoResolve"], cwd=repository)
        self._run(
            ["git", "config", "user.email", "autoresolve@users.noreply.github.com"],
            cwd=repository,
        )
        self._run(
            ["git", "commit", "-m", f"Fix issue #{issue_number} with AutoResolve"],
            cwd=repository,
        )
...
class IssueToPRPipeline:
    ...
    def run(
        ...
        *,
        ...
        branch = f"autoresolve/issue-{issue_number}"
        secrets = tuple(
            value
            for value in (
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="autoresolve-fix-") as temporary:
                ...
                baseline = run_command(
                    ...
                    "git init -q && git config user.name 'AutoResolve' && "
                    "git config user.email 'autoresolve@users.noreply.github.com' && "
                    "printf '%s\\n' '__pycache__/' '*.py[cod]' '.pytest_cache/' "
                    "'.venv/' 'node_modules/' > .git/info/exclude && "
                    "git add -A && git commit -qm 'AutoResolve baseline'",
                    cwd="/workspace/repo",
...
def _pull_request_body(issue: GitHubIssue, strategy: Strategy) -> str:
    return (
        ...
        "Generated by AutoResolve in an isolated Daytona sandbox and verified with the "
